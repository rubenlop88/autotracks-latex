...
\Procedure {EstaEnMovimiento}{$actividad$}
	...
	\ElsIf {$ObtenerEstabaEnMovimiento()$}
		...
		\State \Return {$tiempoTranscurrido \leq tolerancia$}
	\Else
		\State \Return $falso$
	\EndIf
\EndProcedure
...
\Procedure {RegistrarLocalizacion}{$localizacion$}
	\State $enValida \leftarrow EsValida(localizacion)$
	\If {$esValida$}
		\State $GuardarUltimaLocalizacion(localizacion)$
	\EndIf
\EndProcedure
\Statex
\Procedure {EsValida}{$localizacion$}
	\State $ultimaLocalizacion \leftarrow ObtenerUltimaLocalizacion()$
	\State $d \leftarrow CalcularDistancia(localizacion, ultimaLocalizacion)$
	\State $t \leftarrow CalcularTiempo(localizacion, ultimaLocalizacion)$
	\State $v \leftarrow distancia / tiempo$
	\State $p \leftarrow localizacion.precision$
	\State \Return $d > dMinima \land v < vMaxima \land p < pMaxima$
\EndProcedure
\end{algorithmic}
\end{algorithm}
