...
\Procedure{ActividadDetectada}{$actividad$}
	...
	\If {$enVehiculo$}
		\If {El rastreo no está iniciado}
			\State Iniciar el rastreo
		\EndIf
	\Else
		\If {El rastreo está iniciado}
			\State Finalizar el rastreo
		\EndIf
	\EndIf
\EndProcedure
\Statex
\Function {EstaEnVehiculo}{$actividad$}
	...
	\If {$actividad == \text{EN\_VEHICULO}$}
		\State Guardar $t$ como el tiempo de la última detección
		\State \Return $verdadero$
	\EndIf
	\State Inicializar $estabaEnVehiculo$ al último estado detectado
	\If {$estabaEnVehiculo$}
		\State Inicializar $t_0$  al tiempo de la última detección
		\State $\Delta t = t - t_0$
		\State \Return {$\Delta t \leq$ TOLERANCIA}
	\Else
		\State \Return $falso$
	\EndIf
\EndFunction
...
\begin{algorithmic}[1]
\Procedure {RegistrarLocalizacion}{$localizacion$}
	\State $enValida = \Call{EsValida}{localizacion}$
	\If {$esValida$}
		\State Guardar $localizacion$ como la última localización obtenida
	\EndIf
\EndProcedure
\Statex
\Function {EsValida}{$localizacion$}
	\State Inicializar $p$ a la precisión de la $localizacion$
	\State Inicializar $ultimaLocalizacion$ a la última localizacón guardada
	\State $\Delta d = \Call{CalcularDistancia}{localizacion, ultimaLocalizacion}$
	\State $\Delta t = \Call{CalcularTiempo}{localizacion, ultimaLocalizacion}$
	\State $v = \Delta d / \Delta t$
	\State \Return $\Delta d > \text{D\_MINIMA} \land v < \text{V\_MAXIMA} \land p < \text{P\_MAXIMA}$
\EndFunction
\end{algorithmic}
\end{algorithm}
...
\begin{lstlisting}[caption={Obtención de datos de tráfico}, label={lst:candidatos}]
SELECT  id,     -- identificador del segmento de calle 
        source, -- identificador del nodo inicial
        target, -- identificador del nodo final
        km,     -- longitud del segmento de calle
        kmh,    -- limite de velocidad del segmento de calle
        x1,     -- longitud del nodo inicial
        y1,     -- latitud del noto inicial
        x2,     -- longitud del nodo final
        y2,     -- latitud del nodo final
...
Para asignar el peso a cada arista se combinan la \cref{funcion_espacial} y la \cref{funcion_temporal} para definir la función espacio-temporal de la siguiente forma:
...
\Procedure {STMatching}{$G:$ grafo de calles, $T:$ trayectoria}
	\State $G_T = ObtenerCandidatos(G,T)$
	\State $R = ObtenerResultado(G_T)$
	\State \Return $R$
\EndProcedure
\Statex
\Procedure {ObtenerCandidatos}{$G:$ grafo de calles, $T:$ trayectoria}
...
\EndProcedure
\Statex
\Procedure {ObtenerResultado}{$G_T:$ grafo de candidatos}
...
\begin{lstlisting}[caption={Obtención de datos de tráfico}, label={lst:trafico}]
